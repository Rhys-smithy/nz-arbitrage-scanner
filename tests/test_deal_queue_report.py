...
class TestCrossPipelineOverlapNote(unittest.TestCase):
    """Bug 2 fix: Discovery and the legacy Daily Scan independently scrape
    the same Turners categories (scanner/search/auction_search.py reuses
    the legacy pipeline's own scrapers), so the same physical listing can
    be persisted by both and previously rendered as two unrelated-looking
    rows with no indication they're the same real item -- confirmed
    against real persisted data (the same Turners URL appeared in both a
    day's legacy CSV and that day's discovery JSON). This is a
    static-HTML/JS-source regression guard (matching this file's existing
    style for client-side-computed features -- see TestCommandCentreMarkup)
    rather than a headless browser run: it locks in that the source+
    canonical-URL matching logic and its wiring into renderRow() stay
    present, and that neither payload's embedded data is touched by it."""
    ...
    def _persist_discovery_run(self, opportunities, **meta_overrides):
        meta = {
            "run_timestamp": "2026-08-24T09:00:00+00:00", "mode": "discover", "queries_run": 15,
            "candidates_found": len(opportunities), "candidates_verified": len(opportunities),
            "candidates_verification_dropped": 0, "opportunity_count": len(opportunities),
            "decision_counts": {},
        }
        meta.update(meta_overrides)
        path, payload = write_discovery_report(opportunities, meta, reports_dir=self.tmpdir.name)
    ...
    def _persist_legacy_run(self, rows, csv_filename="opportunities_20260824_0900.csv"):
        path = os.path.join(self.tmpdir.name, csv_filename)
        with open(path, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=LEGACY_FIELDNAMES)
            writer.writeheader()
            for row in rows:
                writer.writerow({k: row.get(k, "") for k in LEGACY_FIELDNAMES})
        with open(self.legacy_index_path, "w", encoding="utf-8") as f:
            json.dump({"reports": [{"csv": csv_filename, "timestamp": "2026-08-24T09:00", "rows": len(rows)}]}, f)
    ...
    def test_matching_logic_present_and_wired_into_renderRow(self):
        self._persist_discovery_run([_opportunity(decision="BUY")], decision_counts={"BUY": 1})
        html = self._render()
        self.assertIn("function crossPipelineOther(it) {", html)
        # Reuses the exact same source+canonical-URL identity Hunting
        # already established -- huntingKey() -- rather than inventing a
        # second notion of "same item".
        self.assertIn("var k = huntingKey(it);", html)
        self.assertIn("crossPipelineNote", html)
        self.assertIn("Same listing also found by", html)

    def test_overlapping_url_across_pipelines_is_detected_in_source_data(self):
        # This does not execute the client-side JS (see class docstring),
        # but proves the exact real-world scenario the note is for is
        # genuinely representable end-to-end: the same source+URL persisted
        # by both pipelines, both fully present, unaltered, in the
        # rendered page for the client-side key matching to find.
        shared_url = "https://www.turners.co.nz/General-Goods/Search/computer/other/25309833"
        self._persist_discovery_run(
            [_opportunity(decision="WATCH", url=shared_url)], decision_counts={"WATCH": 1},
        )
        self._persist_legacy_run([
            {"category": "Electronics & Tech", "source": "Turners", "title": "Fuji Copier",
             "url": shared_url, "price_nzd": "1", "score": "6"},
        ])
        html = self._render()
        ...
        discovery_embedded = json.loads(html[start:end])
        self.assertEqual(discovery_embedded["opportunities"][0]["url"], shared_url)
        self.assertEqual(discovery_embedded["opportunities"][0]["source"], "Turners")

        start_marker = '<script id="legacy-scan-data" type="application/json">'
        start = html.index(start_marker) + len(start_marker)
        end = html.index(end_marker, start)
        legacy_embedded = json.loads(html[start:end])
        self.assertEqual(legacy_embedded["rows"][0]["url"], shared_url)
        self.assertEqual(legacy_embedded["rows"][0]["source"], "Turners")

    def test_non_overlapping_items_leave_both_payloads_present_and_unmodified(self):
        # Neither pipeline's persisted record is touched, removed, or
        # merged by this feature -- it is purely an additional display
        # note (see this module's own docstring on why the two pipelines
        # are deliberately never forced into one schema).
        discovery_payload = self._persist_discovery_run(
            [_opportunity(decision="BUY", url="https://www.turners.co.nz/only-in-discovery")],
            decision_counts={"BUY": 1},
        )
        self._persist_legacy_run([
            {"category": "House & Garden", "source": "Turners", "title": "Unrelated Item",
             "url": "https://www.turners.co.nz/only-in-legacy", "price_nzd": "5", "score": "4"},
        ])
        html = self._render()
        ...
        embedded_discovery = json.loads(html[start:end])
        self.assertEqual(embedded_discovery, discovery_payload)
